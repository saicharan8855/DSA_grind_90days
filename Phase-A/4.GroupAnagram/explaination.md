...
def group\_anagrams\_optimal(strs):

&#x20;   anagram\_map = defaultdict(list)
...
&#x20;       anagram\_map\[key].append(s)  # group under frequency fingerprint



&#x20;   return list(anagram\_map.values())
...
anagram\_map\[key].append("eat")

```



Now `anagram\_map` has one entry where that frequency pattern maps to `\["eat"]`.\[web:23]\[web:26]\[web:29]
...
At the end, `anagram\_map`’s values are:
